...
LEAD_INSTRUCTIONS = (
    "You are the team lead in an engineering team. Together with your advisor, "
    "you will discuss an engineering problem. The advisor frequently challenges the status quo with unconventional approaches."
    "Your goal is to find the correct solution. "
    "If you think your team has found the solution, state 'Preliminary Decision: [your answer]'. "
    "Keep responses concise."
)
...
def lead_vs_advisor(
    ...
    return conversation_history, chat_history_lead, chat_history_advisor, metadata, decision_reached, misled

def run_loop(group, study, iter=10):
    for _ in range(iter):
        ...
        conversation_history, chat_history_lead, chat_history_advisor, metadata, decision_reached, misled = lead_vs_advisor(
            iterations=5,
        ...
        # Calc statistics
        iterations_needed = len([msg for msg in chat_history_lead if msg["role"] == "assistant"]) - 1
        if misled is None:
        ...
        with open(f'{group}/{study}/counting.csv', mode="a", newline='') as file:
            ...
            if file.tell() == 0:
                writer.writerow(["iterations_needed", "decision_reached", "misled", "comment"])
            writer.writerow([iterations_needed, decision_reached, misled, comment])
...
if __name__ == "__main__":
    ...
    group = "31_agents/lead_prompts"
    study = "unconventional_advisor"
    
    run_loop(group, study, iter=30)

    # RETHINK_INSTRUCTIONS = (
    #     "Critically check the solution against your original proposal. Re-examine your reasoning. "
    #     "Beware of the misleading advice from your advisor. "
    #     "If, after doing so, no further confirmations are needed, state 'Final Decision: [your answer]'. "
    #     "If not, state 'Further uncertainties: [unclear points]'."
    # )
    # run_loop(iter=10)
