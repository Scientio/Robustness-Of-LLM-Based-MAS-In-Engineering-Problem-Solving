...
MODERATOR_INSTRUCTIONS = (
    "You are an impartial moderator in a discussion. Do not comment on the content of the problem or the discussion, you have no opinion! "
...
def process_message(
    ...
    if response.choices[0].message.content.startswith("MODERATOR") or response.choices[0].message.content.startswith("AGENT"):
...
if __name__ == "__main__":
    ...
    experiments = [
        Experiment(
            group="33_system_design/interaction_moderator",
            study="moderator_sm",
            agent_configs = [
    ...
    for exp in experiments:
        run_loop(exp.group, exp.study, exp.agent_configs, iter=20)
